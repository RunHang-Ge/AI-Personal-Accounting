...
def parse_summary_command(text: str):
    ...
    #如果只输入“/summary"，此时实际的搜索会变为 group=category，等于默认按类别搜索
    ...
    # 如果只输入“/summary"，此时实际的搜索会变为 搜索当前1个月的开支
    else:
...
def parse_update_command(text: str):
    ...
    if len(parts) < 2:
        raise ValueError(
            "修改格式错误。请至少提供记录ID和一个修改字段。\n\n"
            "例如：\n"
            "/update 3 category=交通"
        )
    ...
    for part in parts[1:]:
        ...
        if key not in allowed_fields:
            raise ValueError(
                f"不支持修改字段：{key}\n"
                "支持字段：date、category、amount、currency、merchant、note"
            )
...
async def telegram_webhook(request: Request):
    ...
    if text.startswith("/add"):
        ...
        try:
            ...
            if not row:
                send_message(chat_id, "修改失败：没有找到这条记录，或这条记录不属于你。")
            else:
                reply = format_transaction_detail(row)
                send_message(chat_id, reply)

        except Exception as e:
            send_message(chat_id, f"修改失败：\n{str(e)}")
        ...
        send_message(
            ...
            "例如：\n"
            "/update 3 category=交通\n"
            "/update 3 amount=32.50\n"
            "/update 3 merchant=Grab note=回家\n"
            "/update 3 date=2026-05-30 category=餐饮 amount=18.80"
        )
